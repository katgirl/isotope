...
class IsotopeOrder extends IsotopeProductCollection
{
	...
	private function generateOrderId()
	{
		...
		if ($this->strOrderId == '')
		{
			...
			$objMax = $this->Database->prepare("SELECT order_id FROM tl_iso_orders WHERE " . ($strPrefix != '' ? "order_id LIKE '$strPrefix%'" : '') . " ORDER BY CAST(" . ($strPrefix != '' ? "SUBSTRING(order_id, " . ($intPrefix+1) . ")" : 'order_id') . " AS UNSIGNED) DESC")->limit(1)->executeUncached();
