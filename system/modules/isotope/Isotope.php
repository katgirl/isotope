...
class Isotope extends Controller
{
    ...
    if (!is_numeric($this->Cart->subTotal2)) {
      $fltPrice = $this->Cart->subTotal2 = $this->Cart->grandTotal;      
    } else {
      $fltPrice = $this->Cart->subTotal2;  
    }

		if ($blnPercentage)
		{
			$fltSurcharge = (float)substr($strPrice, 0, -1);
			$fltPrice = $this->Isotope->roundPrice($fltPrice / 100 * $fltSurcharge);
		}
		else
		{
	...
	{
		if (!$this->Database->tableExists('tl_iso_labels'))
		{
			return $label;
		}
	...
	public function formatValue($strTable, $strField, $varValue)
	{
		...
		{
			return $this->formatDate($varValue);
		}

		elseif ($GLOBALS['TL_DCA'][$strTable]['fields'][$strField]['eval']['rgxp'] == 'time')
		{
			return $this->formatTime($varValue);
		}
		...
		{
			return $this->formatDatim($varValue);
		}
	 ...
	 * @return  string
	 */
	public function formatDate($intTstamp)
	{
	 ...
	 * @return  string
	 */
	public function formatTime($intTstamp)
	{
	 ...
	 * @return  string
	 */
	public function formatDatim($intTstamp)
	{
